...
한중일 문자와 표의 문자를 사용하지 않는 대부분의 문자열을 UTF-16보다 더 작은 크기로 표현할 수 있다.

UTF-8은 가변 길이 인코딩이며, 다른 문자는 다른 바이트 수로 표현될 수 있다. 

UTF-8에서는 어떤 문자를 표현하기 위하여 더 많은 바이트가 필요하여, 용량이 커질 수 있다.

* UTF-16 (코드단위 16비트)

JavaScript 해석기는 UTF-16을 사용(용량은 크지만 처리속도가 빠름)

### MDN reference를 보다 빠르게 사용할 수 있는 방법 

참고 : [devdocs] (http://devdocs.io/)

### 느낀점

개념정리는 항상 재밌으면서도, 머리가 터질 것 같다 휴

이제 이것들을 어떻게 코드에 적용시켜볼까가 문제인 것 같당!
