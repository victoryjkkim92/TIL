...
# 10/16(화) 수업 내용 복습 퀴즈  
 ...
 1. 브라우저 상의 모든 이벤트는 캡처링, 버블링 단계의 전파 과정을 거친다 - 아니다, submit과 같은 이벤트는 캡처링이나 버 같은 전파 과정을 거치지 않는다.
...
11. Object.assign 정적 메소드는 객체를 복사하는데 사용되지만, 중첩된 객체까지도 복사하지는 못한다. 
...
8. 접근하고자 하는 속성 이름을 코드 작성 시점에 알 수 없는 경우(사용자로부터 입력 받기 때문에)에는 대괄호 표기법을 사용해 객체의 속성에 접근하는 것이 좋다. 
