...
본격적으로 자바스크립트를 들어가기 전에 기본적인 개념들을 사용한 알고리즘 문제를 풀면서, 최대한 많은 용어들을 접해보려고 했다. 그 중에서도 `NaN`과 같은 값들이 생소했었는데 설명을 읽어보고나니 조금 이해가 된 것 같다.

또 JavaScript의 구동환경에 여러 기능들이 있다는 것을 알게 되었고, 코드를 짜고 확인을 해보기 위해 쓰던 `console.log()`,`alert()`,`prompt()`등이 웹 브라우저에 내장 되어 있는 기능이라는 것을 알게 되었다!
